...
RSpec.describe Evilution::Coverage::MapBuilder do
  ...
  spec   = File.join(fixture_dir, "calculator_spec.rb")
  ...
  it "maps each method's line to the example that exercises it" do
    ...
    expect(add_examples).to include(a_string_matching(/calculator_spec\.rb:\d+/))
    expect(sub_examples).to include(a_string_matching(/calculator_spec\.rb:\d+/))
