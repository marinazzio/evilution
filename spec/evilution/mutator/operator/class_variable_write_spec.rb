...
RSpec.describe Evilution::Mutator::Operator::ClassVariableWrite do
  ...
  describe "#call" do
    ...
    it "replaces exactly the value span when substituting nil" do
      ...
      # would overrun the value and corrupt the surrounding source.
      mutations = described_class.new.call(single_subject)
      nil_mutation = mutations.find { |m| m.mutated_source.include?("= nil") }

      expect(nil_mutation).not_to be_nil
      expect(nil_mutation.mutated_source).to include("    @@total = nil\n    @@total\n  end")
    end
    ...
    it "recurses into the value to mutate a nested class variable write" do
      ...
      nested_subject = subjects.find { |s| s.name.include?("nested_cvar") }
