...
require_relative "method_splice_evaluator"
...
# clear AS::Concern state -> attempt method-body splice on the live owner ->
# fall back to file-level eval inside a redefinition-recovery wrapper.
# Splice avoids re-running class-body side effects (registries, mixin
# registration) that don't tolerate being executed twice in the same process.
...
class Evilution::Integration::Loading::MutationApplier
  def initialize(syntax_validator: Evilution::Integration::Loading::SyntaxValidator.new,
                 ...
                 method_splice_evaluator: Evilution::Integration::Loading::MethodSpliceEvaluator.new,
    ...
    @method_splice_evaluator = method_splice_evaluator
  ...
  def apply(mutation)
    ...
    return if @method_splice_evaluator.call(mutation) == :spliced
